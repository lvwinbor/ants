...
string pathFolder;
double vehicleLength = 0.6; // 车辆长度
double vehicleWidth = 0.6; // 车辆宽度
double sensorOffsetX = 0; // 传感器在车辆坐标系下的X偏移量
double sensorOffsetY = 0; // 传感器在车辆坐标系下的Y偏移量
bool twoWayDrive = true; // 是否支持双向行驶
double laserVoxelSize = 0.05; // 激光点云体素大小
double terrainVoxelSize = 0.2; // 地形点云体素大小
bool useTerrainAnalysis = false; // 是否使用地形分析
bool checkObstacle = true; // 是否检测障碍物
bool checkRotObstacle = false; // 是否检测旋转障碍物
double adjacentRange = 3.5; // 相邻路径的范围
double obstacleHeightThre = 0.2; // 障碍物高度阈值
double groundHeightThre = 0.1; // 地面高度阈值
double costHeightThre = 0.1; // 代价高度阈值
double costScore = 0.02; // 代价分数
bool useCost = false; // 是否使用代价
const int laserCloudStackNum = 1; // 激光点云堆栈数量
int laserCloudCount = 0; // 激光点云数量
int pointPerPathThre = 2; // 每条路径的点数阈值
double minRelZ = -0.5; // 相对高度最小值
double maxRelZ = 0.25; // 相对高度最大值
double maxSpeed = 1.0; // 最大速度
double dirWeight = 0.02; // 方向权重
double dirThre = 90.0; // 方向阈值
...
bool pathScaleBySpeed = true; // 是否根据速度进行路径缩放
double minPathRange = 1.0; // 最小路径范围
double pathRangeStep = 0.5; // 路径范围步长
bool pathRangeBySpeed = true; // 是否根据速度进行路径范围调整
bool pathCropByGoal = true; // 是否根据目标点裁剪路径
bool autonomyMode = false; // 是否处于自主模式
double autonomySpeed = 1.0; // 自主模式下的速度
double joyToSpeedDelay = 2.0; // 从手柄输入到速度生效的延迟时间
double joyToCheckObstacleDelay = 5.0; // 从手柄输入到检测障碍物生效的延迟时间
double goalClearRange = 0.5; // 目标点清除范围
...
// 如果不使用可行驶区域分析，则把点云筛选一下，仅保留到车辆距离小于adjacentrange的点云，存入laserCloudCrop。
...
// 如果使用可行驶区域分析，流程如上，区别是数据存入terrainCloudCrop
...
// 将输入的边界消息中的连续点连接成线段，然后将这些线段按照一定的间隔划分成一系列的点，并将这些点存储在 boundaryCloud 中。
// 这个过程通常用于处理地形或路径规划中的边界信息，以便后续的处理和分析。
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        ...
        if (selectedGroupID >= 0) {
          ...
          path.header.frame_id = antname + "/vehicle";
          ...
          freePaths2.header.frame_id = antname + "/vehicle";
      ...
      plannerCloud2.header.frame_id = antname + "vehicle";
