...
ros::Publisher path2follower;
ros::Publisher pathlist_last;
...
void CubicSplineClass::PlatoonCallback1(const nav_msgs::Odometry::ConstPtr& p_msg){
    int idx = 0;
    ...
    // nav_msgs::Odometry pp_msg = *p_msg;
    // tf::Quaternion RQ3;
    // double roll,pitch,yaw;
    // tf::quaternionMsgToTF(pp_msg.pose.pose.orientation,RQ3);  
    // tf::Matrix3x3(RQ3).getRPY(roll,pitch,yaw);
    // pp_msg.pose.pose.position.x = pp_msg.pose.pose.position.x - 1.5 * cos(yaw);
    // pp_msg.pose.pose.position.y = pp_msg.pose.pose.position.y - 1.5 * sin(yaw);
    // int idx = 0;
    // printf("[cs_planner]: locCallback idx=%i, x=%f, y=%f\n", idx, p_msg->pose.pose.position.x, p_msg->pose.pose.position.y);
    // pList[idx].first = 1;
    // pList[idx].second = pp_msg;
}

void CubicSplineClass::PlatoonCallback2(const nav_msgs::Odometry::ConstPtr& p_msg){

    // tf::Pose tf_odom_pose;
    // tf::poseMsgToTF(p_msg->pose.pose, tf_odom_pose);
    ...
    vehicle_height = p_msg->pose.pose.position.z;
  
    odomTime = p_msg->header.stamp.toSec();
    int idx = 1;
    // printf("[cs_planner]: locCallback idx=%i, x=%f, y=%f\n", idx, p_msg->pose.pose.position.x, p_msg->pose.pose.position.y);
    pList[idx].first = 1;
    pList[idx].second = *p_msg;
...
void CubicSplineClass::PlatoonCallback3(const nav_msgs::Odometry::ConstPtr& p_msg){
    int idx = 2;
    // printf("[cs_planner]: locCallback idx=%i, x=%f, y=%f\n", idx, p_msg->pose.pose.position.x, p_msg->pose.pose.position.y);
    pList[idx].first = 1;
    pList[idx].second = *p_msg;
...
void CubicSplineClass::PlatoonCallback4(const nav_msgs::Odometry::ConstPtr& p_msg){
    int idx = 3;
    // printf("[cs_planner]: locCallback idx=%i, x=%f, y=%f\n", idx, p_msg->pose.pose.position.x, p_msg->pose.pose.position.y);
    pList[idx].first = 1;
    pList[idx].second = *p_msg;
...
void CubicSplineClass::PlatoonCallback5(const nav_msgs::Odometry::ConstPtr& p_msg){
    int idx = 4;
    // printf("[cs_planner]: locCallback idx=%i, x=%f, y=%f\n", idx, p_msg->pose.pose.position.x, p_msg->pose.pose.position.y);
    pList[idx].first = 1;
    pList[idx].second = *p_msg;
    ...
    // pp_msg.pose.pose.position.y = pp_msg.pose.pose.position.y - 1.5 * sin(yaw);
    // int idx = 4;
    // printf("[cs_planner]: locCallback idx=%i, x=%f, y=%f, yaw=%f\n", idx, p_msg->pose.pose.position.x, p_msg->pose.pose.position.y, yaw);
    // pList[idx].first = 1;
    // pList[idx].second = pp_msg;
}
...
void CubicSplineClass::update_formation(double formation_length_unit) {
    ...
    */
    ...
    point_in_leader << 0.0 * formation_length_unit + 0.2, 1.0 * formation_length_unit;
    coneTrans(point_in_leader, formation_diamond[1], move);

    point_in_leader(0) = -1.0 * formation_length_unit + 0.2;
    point_in_leader(1) = 2.0 * formation_length_unit;
    coneTrans(point_in_leader, formation_diamond[2], move);

    point_in_leader(0) = -2.0 * formation_length_unit + 0.2;
    point_in_leader(1) = 3.0 * formation_length_unit;
    coneTrans(point_in_leader, formation_diamond[3], move);
    
    point_in_leader(0) = -3.0 * formation_length_unit + 0.2;
    point_in_leader(1) = 4.0 * formation_length_unit;
    coneTrans(point_in_leader, formation_diamond[4], move);
    ...
    */
    ...
    // point_in_leader << -1.0 * formation_length_unit + 0.2, 0.9 * formation_length_unit;
    // coneTrans(point_in_leader, formation_triangle[1], move);

    // point_in_leader(0) = -1.0 * formation_length_unit + 0.2;
    // point_in_leader(1) = -0.9 * formation_length_unit;
    ...
    point_in_leader << 0.0 * formation_length_unit + 0.2, 0.9 * formation_length_unit;
    coneTrans(point_in_leader, formation_triangle[1], move);

    point_in_leader << -1.0 * formation_length_unit + 0.2, 1.8 * formation_length_unit;
    coneTrans(point_in_leader, formation_triangle[2], move);

    point_in_leader(0) = 0.0 * formation_length_unit + 0.2;
    point_in_leader(1) = -0.9 * formation_length_unit;
    coneTrans(point_in_leader, formation_triangle[3], move);

    point_in_leader(0) = -1.0 * formation_length_unit + 0.2;
    point_in_leader(1) = -1.8 * formation_length_unit;
    coneTrans(point_in_leader, formation_triangle[4], move);
}
...
void CubicSplineClass::planning(){
    ...
    if (!(self_initial && front_initial)) {
        ...
        common_private_msgs::planning_info empty2control;
    ...
    // target.x = target.x - 5 * cos(target.yaw);
    // target.y = target.y - 5 * sin(target.yaw);
    ...
    // std::cout << "[cs_planner]: mystate.x" << mystate.x << std::endl; 
    // std::cout << "[cs_planner]: mystate.y" << mystate.y << std::endl; 
    // std::cout << "[cs_planner]: target.x" << target.x << std::endl; 
    // std::cout << "[cs_planner]: target.y" << target.y << std::endl; 
    // std::cout << "[cs_planner]: midpoint" << midpoint(0) << std::endl; 
    // std::cout << "[cs_planner]: midpoint" << midpoint(1) << std::endl; 
    ...
    // while(count <= 50){
        ...
        // else{
            ...
            common_private_msgs::planning_info trajectory2control;
            nav_msgs::Path path2follower_info;
            common_private_msgs::vehicle_status plan_pose0;
            geometry_msgs::PointStamped path_lastpoint;
            int num0 = r_path1.r_s.size();
            path2follower_info.poses.resize(num0);
            for(int i = 0; i < num0; i++)
            {
                plan_pose0.xPos = r_path1.r_x[i];
                plan_pose0.yPos = r_path1.r_y[i];
                plan_pose0.yaw = r_path1.r_yaw[i];
                plan_pose0.liner_speed = r_path1.r_s[i];
                plan_pose0.angular_speed = r_path1.r_curvature[i];
                trajectory2control.states.push_back(plan_pose0);

                path2follower_info.poses[i].pose.position.x =  r_path1.r_x[ i];
                path2follower_info.poses[i].pose.position.y =  r_path1.r_y[ i];
                path2follower_info.poses[i].pose.position.z = vehicle_height;
                ...
                if(i == num0 - 1)
                {
                    path_lastpoint.point.x = r_path1.r_x[i];
                    path_lastpoint.point.y = r_path1.r_y[i];
                    path_lastpoint.point.z = vehicle_height;
                }
            }
            // path2follower_info.header.frame_id = antname + "/vehicle";
            path2follower_info.header.frame_id = "map";
            path2follower_info.header.stamp = ros::Time().fromSec(odomTime);
            path_lastpoint.header.frame_id = "map";
            path_lastpoint.header.stamp = ros::Time().fromSec(odomTime);
            _trajectory_pub.publish(trajectory2control);
            trajectory_vis_pub.publish(trajectory_vis);
            path2follower.publish(path2follower_info);
            pathlist_last.publish(path_lastpoint);
            path2follower_info.poses.clear();
            // UpdateKeeper(trajectory2control.states.back());
            // break;
        // }
...
int main(int argc,char **argv)
{
    ...
    nh.param<double>("overall_length", overall_length, 0.5);
    // nh.param<std::string>("spline_planner/standard_step", antname, "ant00");
    ...
    nhPrivate.getParam("formation_length_unit", formation_length_unit);
    std::cout << "formation_length_unit=" << formation_length_unit << std::endl;
    nhPrivate.getParam("cubic_spline_hz", cubic_spline_hz);


    ros::Publisher trajectory_pub = nh.advertise<common_private_msgs::planning_info>("trajectory_cubic_spline", 1, true);
    trajectory_vis_pub = nh.advertise<geometry_msgs::PoseArray>("trajectory_cubic_spline_vis", 1, true);
    path2follower = nh.advertise<nav_msgs::Path>("path2",1,true);
    pathlist_last = nh.advertise<geometry_msgs::PointStamped>("way_point",1,true);
    ...
    ros::Subscriber platoon0_sub = nh.subscribe("/ant01/state_estimation", 10, &CubicSplineClass::PlatoonCallback1, myCS);
    ros::Subscriber platoon1_sub = nh.subscribe("/ant02/state_estimation", 10, &CubicSplineClass::PlatoonCallback2, myCS);
    ros::Subscriber platoon2_sub = nh.subscribe("/ant03/state_estimation", 10, &CubicSplineClass::PlatoonCallback3, myCS);
    ros::Subscriber platoon3_sub = nh.subscribe("/ant04/state_estimation", 10, &CubicSplineClass::PlatoonCallback4, myCS);
    ros::Subscriber platoon4_sub = nh.subscribe("/ant05/state_estimation", 10, &CubicSplineClass::PlatoonCallback5, myCS);
