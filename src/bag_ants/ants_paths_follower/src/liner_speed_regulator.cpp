...
bool GetVehicleInfo(int index, common_private_msgs::vehicle_status& out) {
    ...
    out.yaw = yaw;
    out.liner_speed = tmp.twist.twist.linear.x;
    return true;
}
...
double speed_control_1(pidKit& kit1) {
    ...
    if(!formation_type){
        ...
        pid_speed = front_pos.liner_speed + Kp * error;
        ...
        std::cout << "[emlike]: front speed : "<< front_pos.liner_speed << ", ego speed : " << now_pos.liner_speed << ", raw pid_speed = " << pid_speed << std::endl;
        GapPlot(error);
...
double speed_control_2(pidKit& kit2) {
    ...
    if(!formation_type){
        ...
        control_error_d = front_pos.liner_speed - now_pos.liner_speed;
        ...
        std::cout << "front speed : "<< front_pos.liner_speed << ", ego speed : " << now_pos.liner_speed << ", raw output_speed = " << output_speed << std::endl;
    }
    ...
    kit2.last_speed = now_pos.liner_speed;
    return output_speed;
}
...
//     position[0] = now_pos.xPos + (now_pos.liner_speed) * forecast_time * cos(now_pos.yaw);
//     position[1] = now_pos.yPos + (now_pos.liner_speed) * forecast_time * sin(now_pos.yaw);
//     return position;
// }
