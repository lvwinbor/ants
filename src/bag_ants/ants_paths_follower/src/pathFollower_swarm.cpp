...
int main(int argc, char** argv)
{
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      float vehicleXRel = vehicleXRec;
      float vehicleYRel = vehicleYRec;
      ...
      disX = path.poses[pathPointID].pose.position.x - vehicleXRel;
      disY = path.poses[pathPointID].pose.position.y - vehicleYRel;
      float disXX = path.poses[pathPointID].pose.position.x - vehicleXRel;
      float disYY = path.poses[pathPointID].pose.position.y - vehicleYRel;
      disX = cos(vehicleYawRec) * disXX + sin(vehicleYawRec) * disYY;
      disY = -sin(vehicleYawRec) * disXX  + cos(vehicleYawRec) * disYY;
      dis = sqrt(disX * disX + disY * disY);
      ...
      // std::cout << "------------1pathPointID"  << pathPointID << std::endl;
      // std::cout << "------------1pathPointIDx"  << path.poses[pathPointID].pose.position.x << std::endl;
      // std::cout << "------------1pathPointIDy"  << path.poses[pathPointID].pose.position.y << std::endl;}
      ...
      if (pubSkipCount < 0) {
        ...
        // cmd_vel.twist.linear.x = liner_speed;
