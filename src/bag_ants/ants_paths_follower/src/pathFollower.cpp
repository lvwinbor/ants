...
void odomHandler(const nav_msgs::Odometry::ConstPtr& odomIn)
{
  ...
  vehiclePitch = pitch;
  vehicleYaw = yaw;
  vehicleX = odomIn->pose.pose.position.x - cos(yaw) * sensorOffsetX + sin(yaw) * sensorOffsetY;//将传感器中心坐标变换到车辆中心
  ...
  while (status) {
    ...
    if (pathInit) {//是否经过pathHandler进行路径初始化
      ...
      if(trackModeAble == false) {//如果不是跟踪模式
        ...
        else vehicleYawRate = -yawRateGain * dirDiff;//在正常速度下的转向,比例控制
      } else {//如果是跟踪模式
      ...
      if (fabs(dirDiff) < dirDiffThre && dis > stopDisThre) {
        ...
        else if (vehicleSpeed > joySpeed3) vehicleSpeed -= maxAccel / 100.0;//如果车辆当前速度高于（joySpeed3），减少车辆速度
      ...
      if (pubSkipCount < 0) {//如果 pubSkipCount 小于0，表示已经达到了发布新消息的时间
        ...
        else cmd_vel.twist.linear.x = vehicleSpeed;//否则将目标速度发送
