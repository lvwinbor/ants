...
UniformGrid::UniformGrid(
  ...
  // resolution_ = size / 3;
  for (int i = 0; i < 2; ++i) {
    int num = ceil(size[i] / grid_size); // 向上取整
...
/**
 * 初始化网格数据，根据配置的参数（如网格大小、最小未知单元数等）创建网格，并计算每个网格的边界和中心点
*/
...
void UniformGrid::updateGridData(const int& drone_id, vector<int>& grid_ids, vector<int>& parti_ids,
  ...
  if (!initialized_) {
    ...
    if ( level_ == 1) grid_ids = relevant_id_;
    // else
    //   grid_ids = {};
    ROS_WARN("Init grid allocation SSBB.");
    initialized_ = true;
  } else {
