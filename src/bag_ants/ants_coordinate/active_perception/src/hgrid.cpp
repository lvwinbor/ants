...
void HGrid::updateGridData(const int& drone_id, vector<int>& grid_ids, bool reallocated,
  ...
  // ROS_ERROR("updateGridDataupdateGridDataupdateGridData: %i", drone_id);
  // // 打印所有输入参数
  // std::cout << "drone_id: " << drone_id << std::endl;
  // std::cout << "reallocated: " << reallocated << std::endl;
  // std::cout << "last_grid_ids: ";
  // for (auto id : last_grid_ids)
  //   std::cout << id << ", ";
  // std::cout << "" << std::endl;
  // std::cout << "grid_ids: ";
  ...
  getConsistentGrid(last_grid_ids, grid_ids, first_ids, second_ids,2);
}

void HGrid::getConsistentGrid(const vector<int>& last_ids, const vector<int>& cur_ids,
    vector<int>& first_ids, vector<int>& second_ids, int is_self) {
    
  // ROS_WARN("getConsistentGridgetConsistentGridgetConsistentGrid: %i", is_self);
  // for(int i = 0; i < last_ids.size(); ++i) {
  //   ROS_WARN("11//last_ids: %i", last_ids[i]);
  // }

  // for(int i = 0; i < cur_ids.size(); ++i) {
  //   ROS_WARN("11//cur_ids: %i", cur_ids[i]);
  // }

  // for(int i = 0; i < first_ids.size(); ++i) {
  //   ROS_WARN("11//first_ids: %i", first_ids[i]);
  // }

  // for(int i = 0; i < second_ids.size(); ++i) {
  //   ROS_WARN("11//second_ids: %i", second_ids[i]);
  // }
  ...
  // for(int i = 0; i < last_ids.size(); ++i) {
  //   ROS_WARN("//last_ids: %i", last_ids[i]);
  // }

  // for(int i = 0; i < cur_ids.size(); ++i) {
  //   ROS_WARN("//cur_ids: %i", cur_ids[i]);
  // }

  // for(int i = 0; i < first_ids.size(); ++i) {
  //   ROS_WARN("//first_ids: %i", first_ids[i]);
  // }

  // for(int i = 0; i < second_ids.size(); ++i) {
  //   ROS_WARN("//second_ids: %i", second_ids[i]);
  // }
...
// 专用
void HGrid::getGridTour(const vector<int>& ids, const Eigen::Vector3d& pos,
    vector<Eigen::Vector3d>& tour, vector<Eigen::Vector3d>& tour2, bool is_pub) {
  const int grid_num1 = grid1_->grid_data_.size();
  auto ids_tmp = ids;
  ids_tmp.erase(ids_tmp.begin());
  ...
  // 必须搜索成功
  // bool optimistic = plan_num_ < 2;
  tour2 = { pos };
  // path_finder_->reset();
  // if (path_finder_->search(pos, centers[0], optimistic) == Astar::REACH_END) {
  //     ROS_ERROR("No path to next viewpoint");
  // } else {
  //     auto path = path_finder_->getPath();
  //     tour2.insert(tour2.end(), path.begin() + 1, path.end());
  // }
  // Vector3d pos_tmp = pos;
  // pos_tmp(2) = centers[0](2);
  // if (path_finder_->search(pos_tmp, centers[0], 1) == Astar::REACH_END) {
  //     auto path = path_finder_->getPath();
  //     tour2.insert(tour2.end(), path.begin() + 1, path.end());
  // } else {
  //   int num_fail2 = 0;
  ...
  //         } else {
  //           num_fail2++;
  //             ROS_ERROR("No path to next point, try again.");
  //         }
  //     }

  //     if(20 == num_fail2) {
  //         ROS_ERROR("No path to next point, give up.");
  //         tour2.push_back(centers[0]);
  //     }
  //     // tour2.push_back(centers[0]);
  // }
  // ROS_ERROR("------------------------------------------------------------------------");
  // ROS_ERROR("pos:%d,%d,%d.", pos(0), pos(1), pos(2));
  // ROS_ERROR("centers[0]:%d,%d,%d.", centers[0](0), centers[0](1), centers[0](2));
  // cout << "centers[0]: " << centers[0].transpose() << endl;
  // cout << "pos: " << pos.transpose() << endl;
  // ROS_ERROR("------------------------------------------------------------------------");
  for (int i = 0; i < 2; ++i) {
    path_finder_->reset();
    if (path_finder_->search(centers[i], centers[i + 1], 1) == Astar::REACH_END) {
      auto path = path_finder_->getPath();
      tour2.insert(tour2.end(), path.begin() + 1, path.end());
    } else {
      int num_fail = 0;
...
void HGrid::getFrontiersInGrid(const vector<int>& grid_ids, vector<int>& ftr_ids, vector<int>& ftr2_ids) {
  ftr_ids.clear();
  ftr2_ids.clear();
  int tmp = grid_ids.front();
  int tmp2 = grid_ids.size();
  int grid_num1 = grid1_->grid_data_.size();
  // tmp2 = grid_ids.size()/2 == 0 ? 1: grid_ids.size()/5;
  tmp2 = grid_ids.size();
  // tmp = grid_ids.size()/4 == 0 ? grid_ids.size(): grid_ids.size()/4;
  ...
  //   ftr2_ids.push_back(grid1_->grid_data_[grid_ids[i]].contained_frontier_ids_.size());
  // }
...
// tmp = 0;
  ...
  for(int i = 0; i < tmp2; ++i) {
    ...
    ftr2_ids.push_back(grid.contained_frontier_ids_.size());
    // if (tmp < grid_num1) {
    //   auto& grid = grid1_->grid_data_[tmp];
    //   for (auto pair : grid.contained_frontier_ids_) ftr2_ids.push_back(pair.first);
    // } else {
    //   // Find all frontier in the same level 1 grid
    //   tmp -= grid_num1;
    //   int coarse;
