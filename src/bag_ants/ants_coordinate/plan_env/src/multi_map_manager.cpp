...
// ROS_ERROR("Drone %d,%d init!!!!!!!!!!!!!!",drone_id_, map_num_);
...
void MultiMapManager::stampTimerCallback(const ros::TimerEvent& e) {
  ...
  // ROS_ERROR("Drone %d send stamp!!!!!!!!!!!!!!", drone_id_);

  // ROS_ERROR("Drone %d send stamp??????????", msg.idx_lists.size());
...
void MultiMapManager::chunkTimerCallback(const ros::TimerEvent& e) {
  ...
  // ROS_ERROR("Drone %d init!!!!!!!!!!!!!!",drone_id_);
  // ROS_ERROR("chunk_buffer_ %d init!!!!!!!!!!!!!!",chunk_buffer_.size());

  
  for (int i = 0; i < chunk_buffer_.size(); ++i) {
    auto& buffer = chunk_buffer_[i];
    // ROS_ERROR("chunk_buffer2_ %d init!!!!!!!!!!!!!!",buffer.front().chunk_drone_id);
